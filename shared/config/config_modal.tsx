...
const ToggleInput = (props: ToggleInputProps) => {
    ...
    return <Toggle id={`toggle:${props.storageKey}`} value={props.value} onSelected={(newValue: boolean) => handleToggleChange(newValue)} />;
};
    ...
    const modalRows = structure.map((modalRow, index) => {
        ...
        const element = () => {
            switch (modalRow.type) {
                ...
                case "dropdown":
                    return <Dropdown storageKey={key} value={currentValue} options={modalRow.options} callback={updateItem} />;
                case "slider":
                    return (
                        <SliderInput storageKey={key} value={currentValue} min={modalRow.min} max={modalRow.max} step={modalRow.step} callback={updateItem} />
                    );
            }
        };

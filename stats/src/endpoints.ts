...
export const LASTFM = {
	...
	charts: (key: string, type: string) =>
		`http://ws.audioscrobbler.com/2.0/?method=chart.gettop${type}&api_key=${key}&format=json`,
};
...
export const SPOTIFY = {
	...
	search: (track: string, artist: string) =>
		`https://api.spotify.com/v1/search?q=track:${filter(track)}+artist:${filter(artist)}&type=track`,
	...
	searchalbum: (album: string, artist: string) =>
		`https://api.spotify.com/v1/search?q=${filter(album)}+artist:${filter(artist)}&type=album`,

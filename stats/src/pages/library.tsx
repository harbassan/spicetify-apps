...
const LibraryPage = ({ config }: { config: ConfigWrapper }) => {
  const [library, setLibrary] = React.useState<LibraryProps | 100 | 200 | 300>(
    100
  );
  ...
  const fetchData = async (
    ...
    try {
      ...
      playlists = playlists.sort((a, b) =>
        a.ownedBySelf === b.ownedBySelf ? 0 : a.ownedBySelf ? -1 : 1
      );
      ...
      const [topArtists, topGenres, topGenresTotal]: any =
        await fetchTopArtists(artists);
      const [ownedTopArtists, ownedTopGenres, ownedTopGenresTotal]: any =
        await fetchTopArtists(ownedArtists);

      const [topAlbums, releaseYears, releaseYearsTotal]: any =
        await fetchTopAlbums(albums);
      const [ownedTopAlbums, ownedReleaseYears, ownedReleaseYearsTotal]: any =
        await fetchTopAlbums(ownedAlbums, topAlbums);
      ...
      Spicetify.LocalStorage.set(
        `stats:library:owned`,
        JSON.stringify(ownedStats)
      );

      console.log(
        "total library fetch time:",
  ...
  const statCards = Object.entries(library.audioFeatures).map(
    ([key, value]) => {

...
export const topTracksReq = async (
  time_range: string,
  ...
  if (config.CONFIG["use-lastfm"] === true) {
    ...
    const lastfmData = await apiRequest(
      "lastfm",
      LASTFM.toptracks(user, key, time_range)
    );
  ...
  const likedArray = await checkLiked(
    response.items.map((track: { id: string }) => track.id)
  );
  if (!likedArray) return 200;
...
const TracksPage = ({ config }: { config: ConfigWrapper }) => {
  ...
  const [topTracks, setTopTracks] = React.useState<Track[] | 100 | 200 | 300>(
    100
  );

...
const GenresPage = ({ config }: { config: ConfigWrapper }) => {
  ...
  const fetchTopGenres = async (
    ...
    const cacheInfo = JSON.parse(
      LocalStorage.get("stats:cache-info") as string
    );
      ...
      ["artists", "tracks"].map(async (type: string, index: number) => {
        if (cacheInfo[index] === true && !force_refetch) {
          return await JSON.parse(
            LocalStorage.get(`stats:top-${type}:${time_range}`) as string
          );
        }
    ...
    const fetchedArtists = fetchedItems[0].filter(
      (artist: any) => artist?.genres
    );
    const fetchedTracks = fetchedItems[1].filter((track: any) => track?.id);

    const genres: [string, number][] = fetchedArtists.reduce(
      (acc: [string, number][], artist: any) => {
        artist.genres.forEach((genre: string) => {
          ...
          if (index !== -1) {
            ...
            acc.push([
              genre,
              1 * Math.abs(fetchedArtists.indexOf(artist) - 50),
            ]);
          }
        });
        return acc;
  ...
  const obscureTracks = topGenres.obscureTracks.map(
    (track: Track, index: number) => (
      <TrackRow
        index={index + 1}
        {...track}
        uris={topGenres.obscureTracks.map((track) => track.uri)}
      />
    )
  );

...
const PlaylistPage = ({ uri }: { uri: string }) => {
  ...
  // @ts-expect-error
  const { Router, Route, Routes, PlatformProvider, StoreProvider } =
    ReactComponent;
  ...
  const fetchData = async () => {
    ...
    const playlistMeta = await apiRequest(
      "playlistMeta",
    ...
    const [topArtists, topGenres, topGenresTotal]: any = await fetchTopArtists(
      artists
    );
    ...
    console.log(
      "total playlist stats fetch time:",
